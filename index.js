...
function initMap() {
  const currentLoc = {lat: 35.227085, lng:  -80.843124};
...
function addMarkerOnCurrentLoc(){
  let marker = new google.maps.Marker(); 
  let campus = ['central', 'levine', 'merancas'];
  for (let i = 0; i < campus.length; i++) {
    listClasses(cpccCampus[campus[i]]);
      marker = new google.maps.Marker({
      position: cpccCampus[campus[i]],
      map: state.map ,
      label: {
...
function calcAndDisplayRoute(dest) {
  ...
  directionsService.route({
      ...
      if (status == 'OK') {
          addTripInfoMarker(dest, response.routes[0].legs[0].distance.text,
...
function listClasses(){
  ...
  for (let i = 0; i < student.class.length; i++) {
     $('#bus-stop-list').append(`<li class = "lists" tabindex ='0' 
                                                data-lat = ${student.class[i].loc.lat}
                                                data-lng = ${student.class[i].loc.lng}>
                              <div class ="listing" data-lat = ${student.class[i].loc.lat} 
                                                    data-lng = ${student.class[i].loc.lng}>
                                <p>
                                  <a href = "#">
                                    <i class="fa fa-bus"></i> ${student.class[i].course} 
                                  </a>
                                  , ${student.class[i].campus}
                                </p>
                              </div>
                          </li>`); 
  }
...
//Add clickable markers on current location
function addTripInfoMarker(destination, distance, duration){
  let marker = new google.maps.Marker(); 
  let tripInfo = distance + ',  ' + duration;
  let image = {
